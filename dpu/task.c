...
inline 
u8 min_reduce(u16* input, u8* lookup_table, u32 len) {
    u16 minimum = lookup_table[input[0]];
    for(u32 i = 1; i < len; ++i) {
        u16 entry = lookup_table[input[i]];
...
inline
void filter_train(u16 min, u8* lookup_table, u32 len) {
    for(u32 i = 0; i < len; ++i) {
        if(lookup_table[i] == min)
            lookup_table[i] = min + 1;
    }
}
...
int fused_kernel() {
    ...
    if (tasklet_id == 0) {
        ...
        // 3. Filters of shape (num_classes, num_filters, filter_entries)
        mat_u8_init(&filters, p.num_filters, p.filter_entries); 
        mram_read_u8(addresses.model, filters.data, p.num_filters * (p.filter_entries * sizeof(u8)));
