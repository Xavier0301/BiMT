...
int main(int argc, char **argv) {
    ...
    u32 max_inputs_per_dpu = dpu_mram_max_inputs(
        &model.p, 
        filters_per_dpu, MODEL_FILTER_SIZE(&model), bits_per_dpu, block_per_dpu
    );
    ...
    components_t size_bytes = (components_t) {
        .model = filters_per_dpu * MODEL_FILTER_SIZE(&model) * sizeof(u8),
...
//             dpu_prediction_bytes);
    ...
    for(u32 model_it = 0; model_it < num_models; ++model_it) {
        for(u32 sample_it = 0; sample_it < samples_per_dpu; ++sample_it) {
            predictions_host[sample_it] = (u8) model_predict2(&model, MATRIX_AXIS1(dataset, sample_it));
        }
    }
    stop(&timer, 3);
